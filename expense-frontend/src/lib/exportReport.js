...
function perDiemLabel(rate) {
  return rate === 25 ? "Domestic" : "International";
}

// ─── Color palette (matches shadcn/ui theme) ────────────────────────────────
const C = {
  primary:    [17, 24, 39],    // hsl(222.2 84% 4.9%) ≈ #111827
  muted:      [107, 114, 128], // muted-foreground
  mutedBg:    [243, 244, 246], // muted/secondary bg
  border:     [229, 231, 235], // border
  white:      [255, 255, 255],
  green:      [22, 163, 74],   // approved badge
  greenBg:    [220, 252, 231],
...
export async function exportReportPdf(report) {
  ...
  const drawRect = (x, yy, w, h, rgb, r = 0) => {
  ...
  // ── Helper: Section card ────────────────────────────────────────────────
  const drawSectionCard = (startY, height) => {
  ...
  const rowH = 7;
  const cardPad = 5;
  const infoCardH = infoFields.length * rowH + cardPad * 2 + 6; // +6 for title
  ...
  if (perDiemDays > 0) {
    ...
    doc.setFont("helvetica", "bold");
    setColor(C.primary);
    doc.text(fmt(perDiemAmount), mx + contentW - 4, y + 12.5, { align: "right" });
  ...
  doc.text(`expense-report-${report.id}`, pw - mx, footerY, { align: "right" });
...
export async function exportReportExcel(report) {
  ...
  const summaryData = [
    ...
    ["Submitter", report.submitterName || "-"],
    ["Approver", report.approverName || "-"],
    ["Approved At", report.approvedAt ? report.approvedAt.replace("T", " ") : "-"],
    ["Comment", report.approvalComment || "-"],
    [],
    ["Per-Diem Rate", perDiemDays > 0 ? `${fmt(perDiemRate)}/day (${perDiemLabel(perDiemRate)})` : "N/A"],
  ...
  ws1["!merges"] = [{ s: { r: 0, c: 0 }, e: { r: 0, c: 1 } }]; // merge title row
